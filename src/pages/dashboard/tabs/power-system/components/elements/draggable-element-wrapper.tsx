...
interface PropsDraggableElementWrapper {
  ...
  gridEnabled?: boolean;
  gridSize?: number;
  elementType?: 'basic-section' | 'detailed-section' | 'visual-section' | 'text';
...
export const DraggableElementWrapper = memo(function DraggableElementWrapper({
  id,
  x,
  y,
  width,
  ...
  const dragStateRef = useRef({
    ...
    resizeMode: null as 'diagonal' | 'horizontal' | 'vertical' | null,
  });

  // Store grid props in refs to avoid stale closures
  const gridPropsRef = useRef({ gridEnabled, gridSize, width, height, elementType });

  // Update refs when props change
  useEffect(() => {
    gridPropsRef.current = { gridEnabled, gridSize, width, height, elementType };
  ...
  const snapToGrid = (posX: number, posY: number): { x: number; y: number } => {
    const { gridEnabled: enabled, gridSize: size, width: w, height: h, elementType: type } = gridPropsRef.current;
    ...
    // For rectangular elements (basic-section, detailed-section, text), snap by top-left corner
    return {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (dragStateRef.current.isDragging) {
        ...
        if (elementType === 'visual-section') {
          ...
          const newSize = Math.max(minWidth, Math.min(maxWidth, dragStateRef.current.startWidth + delta));
          newWidth = newSize;
          newHeight = newSize;
        } else {
          // Standard resize for other element types
          if (direction?.includes('e')) {
            newWidth = Math.max(minWidth, Math.min(maxWidth, dragStateRef.current.startWidth + deltaX));
          }
          ...
          if (!disableVerticalResize) {
            if (direction?.includes('s')) {
              newHeight = Math.max(minHeight, Math.min(maxHeight, dragStateRef.current.startHeight + deltaY));
            }
        ...
        // CRÍTICO: Chamar callback ANTES de atualizar DOM
        // Isso permite que elementos (como TextElement) recalculem dimensões
        onResizeMove?.(newWidth, newHeight, dragStateRef.current.resizeMode ?? undefined);
    ...
    const handleMouseUp = () => {
      if (dragStateRef.current.isDragging) {
        ...
        // Update React state once at the end
        if (onSizeChange) {
          onSizeChange(dragStateRef.current.currentWidth, dragStateRef.current.currentHeight);
        }

        // Save resize mode before clearing it
        const resizeMode = dragStateRef.current.resizeMode;
  ...
  }, [isDragging, isResizing, isEditMode, x, y, width, height, gridEnabled, onPositionChange, onSizeChange, onDragMove, onDragEnd, onResizeMove, onResizeEnd, elementType, minWidth, maxWidth, minHeight, maxHeight, snapToGrid, zoom, disableVerticalResize]);
  ...
  const handleResizeStart = (e: React.MouseEvent, direction: string) => {
    ...
    // Detectar tipo de resize baseado na direção
    ...
    let resizeMode: 'diagonal' | 'horizontal' | 'vertical';
    if (isDiagonal) {
      resizeMode = 'diagonal';
    } else if (isVertical) {
      resizeMode = 'vertical';
          ...
          {elementType === 'visual-section' ? (
            <>
              ...
              <div
                ...
                style={{ top: '-5px', left: '-5px', width: '10px', height: '10px', zIndex: 3 }}
                onMouseDown={(e) => handleResizeStart(e, 'nw')}
              />
              <div
                className="absolute cursor-nesw-resize"
                style={{ top: '-5px', right: '-5px', width: '10px', height: '10px', zIndex: 3 }}
                onMouseDown={(e) => handleResizeStart(e, 'ne')}
              />
              <div
                className="absolute cursor-nwse-resize"
                style={{ bottom: '-5px', right: '-5px', width: '10px', height: '10px', zIndex: 3 }}
                onMouseDown={(e) => handleResizeStart(e, 'se')}
              />
              <div
                className="absolute cursor-nesw-resize"
                style={{ bottom: '-5px', left: '-5px', width: '10px', height: '10px', zIndex: 3 }}
              ...
              {shape === 'circle' && (
                <>
                  <div className="absolute cursor-nwse-resize" style={{ top: '15%', left: '15%', transform: 'translate(-50%, -50%)', width: '12px', height: '12px', zIndex: 2 }} onMouseDown={(e) => handleResizeStart(e, 'nw')} />
                  <div className="absolute cursor-nesw-resize" style={{ top: '15%', left: '85%', transform: 'translate(-50%, -50%)', width: '12px', height: '12px', zIndex: 2 }} onMouseDown={(e) => handleResizeStart(e, 'ne')} />
                  <div className="absolute cursor-nwse-resize" style={{ top: '85%', left: '85%', transform: 'translate(-50%, -50%)', width: '12px', height: '12px', zIndex: 2 }} onMouseDown={(e) => handleResizeStart(e, 'se')} />
                  <div className="absolute cursor-nesw-resize" style={{ top: '85%', left: '15%', transform: 'translate(-50%, -50%)', width: '12px', height: '12px', zIndex: 2 }} onMouseDown={(e) => handleResizeStart(e, 'sw')} />
                </>
              )}
            </>
          ) : (
            <>
              {/* Standard handles for other element types */}
              {/* Visual border: lines connecting corners - hide when editing */}
              {!isEditing && (
                <>
                  <div
                    className="absolute"
              ...
              {!disableVerticalResize && !isEditing && (
                <>
                  {/* Corner visual handles + interaction areas */}
                  {/* Top-left */}
                  <div
                    className="absolute cursor-nwse-resize"
                    style={{
                  ...
                  {/* Top and bottom edge handles - invisible hit areas */}
                  <div
              ...
              {/* Left and right edge handles - invisible hit areas - hide when editing */}
              {!isEditing && (
                <>
                  <div

...
import { FACTION_ALIGNMENTS_CONSTANT } from "@/components/modals/create-faction-modal/constants/faction-alignments";
...
export function AlignmentMatrix({
  ...
  if (!isEditable) {
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <div className="space-y-1">
              <p className="text-sm font-medium text-foreground">
                {t("alignment.no_alignment")}
              </p>
              <p className="text-xs text-muted-foreground">
                {t("alignment.no_alignment_hint")}
              </p>
            </div>
          </div>
        </div>
    ...
    const selectedAlignment = FACTION_ALIGNMENTS_CONSTANT.find(
      (a) => a.value === value
    );

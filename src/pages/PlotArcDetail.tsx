...
import { ArrowLeft, Plus, Edit2, Trash2, Star, CheckCircle2, Circle, Save, X } from "lucide-react";
...
export function PlotArcDetail() {
  ...
  const handleSave = () => {
    if (editForm.name && editForm.focus && editForm.description && editForm.size && editForm.status) {
  ...
  return (
    <div className="min-h-screen bg-background">
                ...
                {isEditing ? (
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-4">
                      <Select 
                        value={editForm.size} 
                        onValueChange={(v) => setEditForm({ ...editForm, size: v as any })}
                      >
              ...
              <div className="flex gap-2">
                {isEditing ? (
                  ...
                  <>
                    ...
                    <Button variant="destructive" onClick={() => setShowDeleteArcDialog(true)}>
      ...
      {/* Delete Arc Dialog */}
      <AlertDialog open={showDeleteArcDialog} onOpenChange={setShowDeleteArcDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Confirmar Exclusão do Arco</AlertDialogTitle>
            <AlertDialogDescription>
              Tem certeza que deseja excluir o arco "{arc.name}"? Esta ação não pode ser desfeita e todos os eventos do arco também serão excluídos.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
            <AlertDialogAction onClick={handleDeleteArc} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
              Excluir
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>

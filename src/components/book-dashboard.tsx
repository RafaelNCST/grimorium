...
import { BookSpeciesTab } from "@/components/tabs/book-species-tab";
import { CharactersTab } from "@/components/tabs/characters-tab";
import { EncyclopediaTab } from "@/components/tabs/encyclopedia-tab";
...
import { PlotTab } from "@/components/tabs/plot-tab";
import { PowerSystemTab } from "@/components/tabs/power-system-tab";
import { WorldTab } from "@/components/tabs/world-tab";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function BookDashboard({ bookId, onBack }: PropsBookDashboard) {
  const { t } = useLanguage();
  ...
  const [arcs, setArcs] = useState<IPlotArc[]>(initialArcs);
  const [isHeaderHidden, setIsHeaderHidden] = useState(false);

  // Tab customization state
  const [isCustomizing, setIsCustomizing] = useState(false);
  const [tabs, setTabs] = useState<ITabConfig[]>(defaultTabs);
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              o nome do livro: <strong>{book.title}</strong>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="my-4">
            <Input

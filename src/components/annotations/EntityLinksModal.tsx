...
import { Link, Unlink, User, Skull, Globe, Mountain, MapPin, Package, Search, Plus } from "lucide-react";
...
import { AnnotationLink, LinkedEntity } from "@/types/annotations";
import { toast } from "@/hooks/use-toast";

interface EntityLinksModalProps {
  isOpen: boolean;
  onClose: () => void;
  noteId: string;
  noteName: string;
  currentLinks: AnnotationLink[];
...
export function EntityLinksModal({ 
  ...
  const getEntityIcon = (type: string) => {
    switch (type) {
      ...
      case 'continent': return <Mountain className="w-4 h-4" />;
  ...
  const isLinked = (entityId: string) => {
    return currentLinks.some(link => link.entityId === entityId);
  };

  const handleToggleLink = (entity: LinkedEntity) => {
    const existingLink = currentLinks.find(link => link.entityId === entity.id);
    
    if (existingLink) {
      // Remove link
      const newLinks = currentLinks.filter(link => link.entityId !== entity.id);
        ...
        <div className="space-y-4">
          ...
          {linkedEntities.length > 0 && (
            <div className="border rounded-lg p-4 bg-muted/20">
              <h3 className="font-semibold mb-2">Links Atuais ({linkedEntities.length})</h3>
              ...
              <ScrollArea className="h-[400px] pr-4">
                <div className="space-y-2">
                  {filteredEntities.map((entity) => (
                    <Card key={entity.id} className="hover:shadow-sm transition-shadow">
                      <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                          ...
                          <Button
                            variant={isLinked(entity.id) ? "destructive" : "outline"}
                            size="sm"
                            onClick={() => handleToggleLink(entity)}
                          >
                            {isLinked(entity.id) ? (

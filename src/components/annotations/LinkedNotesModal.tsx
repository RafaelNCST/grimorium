...
import { FileText, ExternalLink, Calendar, User, Skull, Globe, Mountain, MapPin, Package } from "lucide-react";
...
interface LinkedNotesModalProps {
  isOpen: boolean;
  onClose: () => void;
  entityId: string;
  entityName: string;
        ...
        <ScrollArea className="h-[60vh] pr-4">
          {linkedNotes.length === 0 ? (
            <div className="text-center py-8">
              ...
              <h3 className="text-lg font-semibold mb-2">Nenhuma anotação linkada</h3>
              <p className="text-muted-foreground">
                Você pode criar links para anotações nas abas de anotações ou no editor de arquivos.
              </p>
            </div>
          ) : (
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div className="flex items-center justify-between text-xs text-muted-foreground">
                      ...
                      <Badge variant="secondary" className="text-xs">
                        Linkado em {note.linkCreatedAt.toLocaleDateString('pt-BR')}
                      </Badge>
                    </div>
                  </CardContent>
                </Card>
